...
1. Regex korjattu (ryhmänumerot olivat väärin)
1. Supabase kutsut database.py:n kautta (ei suoraan)
1. Tuntematon alue → DISPATCH_AREA_FALLBACKS-kartta
1. save_to_history käyttää insert_many:a duplikaattien välttämiseksi
1. Signal import lisätty
1. Duplikaattiavain “jäähalli” poistettu (esiintyi kahdesti: rivi 71 ja 84)
1. Asemakartoitukset korjattu:
...
from taxiapp.base_agent import BaseAgent, AgentResult, Signal
from taxiapp.agents.document_reader import (
read_document, DocumentResult, detect_type, capabilities,
)
from taxiapp.areas import AREAS
from taxiapp.config import config
...
try:
from taxiapp.repository.database import DispatchStationRepo
...
```
# 2. Fuzzy-match tapahtumapaikkaan
...
“””
try:
from taxiapp.repository.database import DispatchHistoryRepo
...
async def fetch(self) -> AgentResult:
    ...
    """
    try:
        from taxiapp.repository.database import DispatchSnapshotRepo
