...
app.py:n silta kopioi st.secrets → os.environ käynnistyksessä.
“””
...
fmi_api_url: str = field(
    default_factory=lambda: os.environ.get("FMI_API_URL", "https://opendata.fmi.fi/wfs")
)
digitraffic_mqtt_url: str = field(
    default_factory=lambda: os.environ.get("DIGITRAFFIC_MQTT_URL", "wss://rata.digitraffic.fi/mqtt")
)
digitraffic_rest_url: str = field(
    default_factory=lambda: os.environ.get("DIGITRAFFIC_REST_URL", "https://rata.digitraffic.fi/api/v1")
)
finavia_api_url: str = field(
    default_factory=lambda: os.environ.get("FINAVIA_API_URL", "https://api.finavia.fi/flights/public/v0")
)
...
hsl_rss_url: str = field(
    default_factory=lambda: os.environ.get("HSL_RSS_URL", "https://www.hsl.fi/fi/rss/hairiot")
)
fintraffic_rss_url: str = field(
    default_factory=lambda: os.environ.get("FINTRAFFIC_RSS_URL", "https://liikennetilanne.fintraffic.fi/rss")
)
...
“”“Pakollinen ympäristömuuttuja — kaatuu selkeällä viestillä.”””
value = os.environ.get(key)
if not value:
raise EnvironmentError(
f”[Config] Pakollinen ympäristömuuttuja puuttuu: {key}\n”
f”Aseta Streamlit Cloud → Secrets tai .streamlit/secrets.toml”
)
return value

# Singleton — importataan kaikkialla: from taxiapp.config import config

config = Config()

# ══════════════════════════════════════════════════════════════

# AIKAVYÖHYKEAPUTYÖKALUT

# ══════════════════════════════════════════════════════════════

# VANHA (BUGI): time.daylight — vakio, ei kerro onko kesäaika NYT.

# UUSI: zoneinfo laskee oikean offsetin aina automaattisesti.

HELSINKI_TZ = ZoneInfo(“Europe/Helsinki”)

def helsinki_now() -> datetime:
“”“Palauta Helsingin paikallisaika (timezone-aware).”””
return datetime.now(HELSINKI_TZ)

def helsinki_offset() -> int:
“”“Palauta Helsingin UTC-offset (2 talvella, 3 kesällä).”””
now = datetime.now(timezone.utc)
return int(now.astimezone(HELSINKI_TZ).utcoffset().total_seconds() / 3600)

def to_helsinki(dt: datetime) -> datetime:
“”“Muunna mikä tahansa datetime Helsinki-aikaan.”””
