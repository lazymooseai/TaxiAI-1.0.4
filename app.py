...
Salaisuudet asetetaan Streamlit Cloud → Secrets (TOML-muoto):
SUPABASE_URL       = “https://xxxx.supabase.co”
SUPABASE_ANON_KEY  = “eyJ…”  tai  “sb_publishable_…”
ADMIN_PASSWORD     = “…”     (valinnainen)
...
# ── Python-polku: varmista että taxiapp-paketti löytyy ───────

# Streamlit Cloud ajaa app.py:tä hakemistossa /mount/src/<repo>/

# mutta ei aina lisää sitä sys.path:iin automaattisesti.

_app_dir = str(Path(**file**).resolve().parent)
...
# SILTA: st.secrets → os.environ

# ══════════════════════════════════════════════════════════════

# Streamlit Cloud tallentaa salaisuudet st.secrets-objektiin,

# mutta config.py ja agentit lukevat os.environ.get():llä.

# Tämä silta kopioi ne kerran käynnistyksessä.

# Ei ylikirjoita olemassa olevia env-arvoja (CI/paikallinen .env).
...
# ── Tarkista pakolliset salaisuudet VASTA sillan jälkeen ─────
...
if _missing:
st.error(
f”⚠️ Puuttuvat salaisuudet: `{'`, `'.join(_missing)}`\n\n”
“**Streamlit Cloud** → App settings → Secrets:\n”
“`toml\n" 'SUPABASE_URL      = "https://xxx.supabase.co"\n' 'SUPABASE_ANON_KEY = "sb_publishable_...koko-avain-yhdellä-rivillä"\n' "`\n\n”
“**Paikallinen** → `.streamlit/secrets.toml` (sama muoto)”
)
st.stop()
...
with st.expander("📍 Aseta sijainti käsin", expanded=False):
    ...
    if st.button("Aseta sijainti", key="btn_set_loc"):
        from taxiapp.location import update_driver_location
...
with tabs[0]:
try:
from taxiapp.ui.dashboard import render_dashboard
render_dashboard()
...
with tabs[1]:
try:
from taxiapp.ui.events_tab import render_events_tab
...
with tabs[2]:
try:
from taxiapp.ui.links_tab import render_links_tab
cached = st.session_state.get(“hotspot_cache”)
results = cached[1] if cached else []
...
with tabs[3]:
try:
from taxiapp.ui.stats_tab import render_stats_tab
cached  = st.session_state.get(“hotspot_cache”)
results = cached[1] if cached else []
render_stats_tab(
results,
...
with tabs[4]:
try:
from taxiapp.ui.settings_tab import render_settings_tab
render_settings_tab(
driver_id=st.session_state.get(“driver_id”),
)
except Exception as e:
...
with tabs[5]:
try:
from taxiapp.ui.admin_tab import render_admin_tab
render_admin_tab(
driver_id=st.session_state.get(“driver_id”),
)
except Exception as e:
