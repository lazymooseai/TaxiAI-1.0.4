...
from taxiapp.areas import AREAS, Area
from taxiapp.location import (
...
async def run(self) -> tuple[list[Hotspot], list[AgentResult]]:
    ...
    try:
        from taxiapp.location import get_driver_location
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            ...
            from taxiapp.base_agent import AgentResult as AR
...
def _hospital_boost(
    ...
    try:
        from taxiapp.repository.database import HospitalRepo
...
def _save_snapshot(self, hotspots: list[Hotspot]) -> None:
    ...
    try:
        from taxiapp.repository.database import HotspotRepo
        rows = [
            {
                "rank":    h.rank,
                "area":    h.area,
...
```
agent_classes = [
    ("taxiapp.agents.disruptions", "DisruptionAgent"),
    ("taxiapp.agents.weather",     "WeatherAgent"),
    ("taxiapp.agents.trains",      "TrainAgent"),
    ("taxiapp.agents.flights",     "FlightAgent"),
    ("taxiapp.agents.ferries",     "FerryAgent"),
    ("taxiapp.agents.events",      "EventsAgent"),
    ("taxiapp.agents.social_media","SocialMediaAgent"),
]
