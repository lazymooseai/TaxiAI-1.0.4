...
1. terminal_ typo → terminal_data: list[dict]
1. result. ilman .data → result.data + guard
1. datetime.now() → datetime.now(timezone.utc)
1. Duplikaattiavain A79 poistettu
1. Ei-AREAS alueet kartoitettu lähimpään
1. map_code_to_area: vain metodi
1. preorder_patterns VIEW → suodatus Pythonissa
1. st.session_state poistettu agenteista
   “””
...
async def calculate_advance_warnings(
    ...
    """
    from taxiapp.repository.database import PreorderRepo
...
from taxiapp.config import helsinki_offset
