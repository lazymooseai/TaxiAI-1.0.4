...
drivers: list[dict] = []
try:
    from taxiapp.repository.database import DriverRepo
...
sources: list[dict] = []
try:
    from taxiapp.repository.database import AgentSourcesRepo
...
if st.button("🔄 Tarkista tietokanta", key="btn_health_check"):
    with st.spinner("Tarkistetaan..."):
        try:
            from taxiapp.repository.database import health_check
            result = health_check()
...
with st.expander("📋 SQL-skeema (kopioi Supabase SQL Editoriin)", expanded=False):
    try:
        from taxiapp.repository.database import SCHEMA_SQL
...
```
from taxiapp.areas import all_area_names
...
with col4:
    if st.button("➕ Lisää", key="add_ride_btn", use_container_width=True):
        ...
        if eff_driver:
            try:
                from taxiapp.repository.database import RidesRepo
